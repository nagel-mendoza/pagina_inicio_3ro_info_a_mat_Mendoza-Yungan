...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const accountBtn = document.querySelector('a[href="#cuenta"]') || document.getElementById('accountIconBtn');
    ...
    function checkAuth() {
        try {
            return JSON.parse(localStorage.getItem('molecule_user'));
        } catch (e) {
            return null;
        }
    }
    ...
    // Cerrar menú o modal al hacer clic fuera
    document.addEventListener('click', (e) => {
        // Cerrar menú desplegable del usuario
        if (accountBtn && userDropdown && !accountBtn.contains(e.target) && !userDropdown.contains(e.target)) {
            userDropdown.classList.remove('active');
        }

        // Cerrar modal al presionar el fondo oscuro
